...
- **A moderation-rejected response is a retryable error, not a run-ending one.** Both adapters map a wire `content_filter` finish reason and safety-gate wording (dashscope-intl's `Output data may contain inappropriate content.`) to `finish {kind:'error'}` with the canonical `CONTENT_FILTERED` code, and `dsh-llm-retry` retries it by default; see [content-filtered outputs are retryable](../../.agents/notes/implemented/bug-fix/2026-08-23-content-filtered-output-is-retryable.md).
