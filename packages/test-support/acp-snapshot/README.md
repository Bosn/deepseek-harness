...
`workspaceOutputSnapshots` captures complete generated-workspace text artifacts after graceful application shutdown and before workspace cleanup. Each `path` is cwd-relative and may name a nested artifact; each `snapshot` is a unique direct filename in that scenario directory. Suite registration rejects traversal, path separators, empty names, duplicate snapshot names, and collisions with suite-owned fixtures such as `input.json`, stdout/session outputs, and prompt/schema sidecars before any scenario or refresh write runs. Every mode compares the captured text with that file, while `refresh` writes the complete current text back before comparing it.
