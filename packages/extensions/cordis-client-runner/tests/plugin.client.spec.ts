...
interface Bench {
  ...
  connect: () => void
  /** Retract the current generation as the real Connection source does. */
  disconnect: () => void
...
async function boot(): Promise<Bench> {
  ...
  return {
    ...
    connect: () => {
      publishGeneration({ id: ++generationId, host: { home: '/home/fixture' } })
      // API Gateway emits this cache notification after Connection publishes
      // the generation. It must not cause a second inspect-manifest baseline.
      Reflect.apply(ctx.emit.bind(ctx), undefined, ['connection/reset'])
    },
    disconnect: () => { publishGeneration(undefined) },
...
describe('browser half', () => {
  it('waits for readiness and syncs exactly once for every Connection generation', async () => {
