...
inspect registry 订阅 `ctx.connection.generation`。第一次握手尚未完成时，provider effect 可以先登记；runner 会为每个 ready generation 恰好排队一次完整基线 manifest。generation 丢失会取消进行中的 inspect query，并使排队中的 sync continuation 失效；下一代会重新发送完整快照。因此重连或页面 remount 不会在没有 active Connection 时调用 Remote。
