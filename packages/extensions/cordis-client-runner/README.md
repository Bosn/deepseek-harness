...
The inspect registry subscribes to `ctx.connection.generation`. Provider effects may register while the first handshake is pending, and the runner queues exactly one complete baseline manifest for each ready generation. Losing a generation cancels in-flight inspect queries and invalidates queued sync continuations; the next generation sends a fresh complete snapshot. A reconnect or page remount therefore never calls a Remote without an active Connection.
