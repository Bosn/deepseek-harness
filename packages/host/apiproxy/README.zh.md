...
`session.history` 会读取已附加 Session 的内存状态，或通过持久化检查冷日志，而不会恢复或发布 agent，然后按追加来源的消息边界分页：`maxMessages` 统计以追加方式进入 surface 的 `user/message` 和 `assistant/message` 事件，因此仅供模型使用的替换副本不占用配额。每一页仍是一段连续的原始事件区间，从而让压缩（compaction）的仅日志 `compaction/summary` 记录与引用它的替换留在同一页。页面的序列化大小还单独受 `historyPageMaxBytes` 限定（默认 2 MiB；预算为 0 即关闭该界限），按完整响应计算——RPC 信封、尾页 `projections` 块、数组分隔符与条目字节：网关从页头丢弃完整的消息组——chunk 与工具伴随事件通过 `sourceEventSeqs` 跟随所属消息——因此刷新超大会话时只返回最近的尾部内容，且只要被服务头部之前还有任何内容就应答 `hasMore`（保留的 seq 区间保持连续，连续发出的 `loadOlder` 请求仍能一路向前回溯全部历史）。仍然装不下的页面会摘掉 `projections` 块再限界（与 loadOlder 页一样按能力缺失处理）。最新的消息组即使单独超过预算也会完整保留，因为超预算的一页胜过空页。
