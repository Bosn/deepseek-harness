...
export interface SessionsApi {
   ...
   * Pages are byte-bounded by the deployment's `historyPageMaxBytes`, counted on the complete
   * serialized response (RPC envelope, the `projections` block, event-array delimiters, and entry
   * bytes): a page above the budget drops its oldest whole message groups — never its newest one —
   * until it fits; the dropped content pages back in through beforeSeq, reflected as `hasMore: true`.
   * A page that cannot fit even then omits the `projections` block for that read (capability-absent,
   * the same shape loadOlder pages serve).
