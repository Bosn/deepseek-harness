...
Set `browserSessionAuth: false` to switch the session layer off at its entry points. `BrowserAuth` then runs in bypass mode: root and index requests serve unconditionally, every Host request that clears the trust fence dispatches, the workspace-file listener accepts, and `dsh-web-app` prints a clean, token-free URL. The Host/Origin trust fence above still applies, because it is reachability policy rather than identity; configure `trustedHosts` and confine the network layer (bind, reverse proxy, or VPN) instead. The default remains launch-token plus signed-cookie authentication.

The Host injects `privilegedHosts` as `__DSH_PRIVILEGED_HOSTS__`. Connection derives `ctx.connection.canUseHostConfiguration`, and API Gateway reflects it as `ctx.remote.$host.canUseHostConfiguration` for ordinary Client consumers deciding whether to mount Host-backed settings, credentials, model-provider, and related configuration surfaces on a remote page. Loopback pages and transports that own their Host always expose them; a served remote page does so only when its exact authority matches the injected declaration. This is a deployment UI capability, not an API access-control list: each declared authority joins the ordinary Host/Origin trust fence, and every request still requires the same signed browser session as every other Host operation.
