...
- **没有 Host 配置能力的页面不提供持久化设置**：当 `canUseHostConfiguration` 为假时，本 Client 在那里禁用 Host 持久化，因此表单以 `unavailable` 起步且从不跨线路；尽管 Connection 认证覆盖 API，表单写入仍在那里无效。共享的开发者工具偏好单独提供浏览器本地变更。Connection 认证是另一项独立关注点，并不隐含这项界面能力。
