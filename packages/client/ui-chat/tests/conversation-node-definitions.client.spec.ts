...
/** Settled final Assistant node of one snapshot, when the step produced one. */
...
describe('built-in conversation node Definitions', () => {
  ...
  it('restores first-token metrics after live settlement and after reopening the same history', () => {
    ...
    for (const current of [value, reopened]) {
      ...
      // The live delta set the first-token time; reopening the same history
      // derives it from the settled embedded stream instead of dropping it.
      expect(assistant?.timing).toEqual({
        stepStartTime: 1_010, firstTokenTime: 1_030, completedTime: 1_050,
      })
