...
`ctx.remote.$host` 以普通值读取固定的 Host 事实：`home`（首个 ready 帧之前为 undefined）、`isLoopback` 与 `canUseHostConfiguration`。最后一项事实让显式声明的远程页面使用 Host 支撑的配置，而不会启用桌面专属行为。它不是存储——没有订阅、没有代次计数——所以需要响应重连的消费方去监听 `connection/reset`，而不是轮询它。
