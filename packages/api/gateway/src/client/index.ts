...
export interface RemoteHostFacts {
  ...
  /** Whether this deployment exposes Host-backed configuration surfaces. */
...
class ClientRemoteService extends Service implements ClientRemote {
  ...
  get $host(): RemoteHostFacts {
    ...
    // itself changed. The two deployment facts are fixed for the page lifetime.
    ...
    if (this.hostFacts === undefined || this.hostFacts.home !== home) {
      this.hostFacts = {
        ...
        canUseHostConfiguration: this.connection.canUseHostConfiguration,
      }
    }
