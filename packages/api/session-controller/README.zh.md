...
Host 按完整的浏览器载体 JSON 大小限制每个历史页与 follow opening snapshot。该完整载体保证适用于仓库的标准 Web clients；它们的 Connection RPC id 与 Gateway stream id 都是 36 字节 UUID，使用其他关联 id 长度的 raw 或 custom carrier 不在该保证范围内。预算包含 Connection 响应或 Gateway stream 信封、record 分隔符与 UTF-8 字节、header 字段，以及 opening projection baseline。按消息数限定的窗口过大时，controller 会从页头移除最旧的完整 append-message 组；`sourceEventSeqs` 使 packed chunk 和工具配套事件跟随所属消息，保留的序列区间仍连续，`hasMore` 则标记每个被移除的前缀。如果 opening projection baseline 使最新消息组无法装入预算，controller 会在该 generation 省略这个不可分割的 baseline。最新消息组即使单独超过预算也会整组保留。`historyPageMaxBytes` 默认为 2 MiB，`0` 禁用该字节限制。
...
| `historyPageMaxBytes` | `2,097,152` | 历史页或 opening snapshot 的完整浏览器载体 JSON 大小上限；`0` 禁用该限制 |
