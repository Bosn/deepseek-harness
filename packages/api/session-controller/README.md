...
The Host limits each history page and follow opening snapshot by the complete browser-carrier JSON size. This complete-carrier guarantee applies to the repository's standard Web clients, whose Connection RPC ids and Gateway stream ids are 36-byte UUIDs; raw or custom carriers with other correlation-id lengths are outside the guarantee. The budget includes the Connection response or Gateway stream envelope, record delimiters and UTF-8 bytes, header fields, and the opening projection baseline. When a count-bounded window is too large, the controller removes oldest whole append-message groups from its head; `sourceEventSeqs` keeps packed chunks and tool companions with their message, the retained sequence range stays contiguous, and `hasMore` advertises every removed prefix. If the opening projection baseline prevents the newest group from fitting, the controller omits that indivisible baseline for the generation. The newest group remains whole even when it alone exceeds the budget. `historyPageMaxBytes` defaults to 2 MiB, and `0` disables this byte bound.
...
| `historyPageMaxBytes` | `2,097,152` | Maximum complete browser-carrier JSON size for a history page or opening snapshot; `0` disables the bound |
