...
type ProjectedSessionSnapshotFrame = SessionSnapshotFrame & {
  readonly projections: NonNullable<SessionSnapshotFrame['projections']>
}
...
async function opening(
  ...
  sessionId: SessionId,
  maxMessages?: number,
): Promise<ProjectedSessionSnapshotFrame> {
  ...
  if (projections === undefined) throw new Error('follow opening snapshot omitted its projection baseline')
  return { ...first.value, projections }
}
