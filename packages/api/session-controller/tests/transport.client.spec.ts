...
type SessionSnapshotFrame = Extract<SessionFollowFrame, { type: 'snapshot' }>

const ADDRESS = { kind: 'session', sessionId: 'session-1' as never } satisfies SessionAddress
...
describe('Session Client stream adapters', () => {
  it('opens a journal when the byte-bounded snapshot omits projections', async () => {
    const opening: SessionSnapshotFrame = {
      type: 'snapshot',
      header: {
        version: 0,
        id: ADDRESS.sessionId,
    ...
    expect(changes[0]).toMatchObject({ type: 'replace', entries: [entry(0)] })
    expect(changes[0]?.type === 'replace' ? changes[0].page : undefined)
      .not.toHaveProperty('projections')
