...
describe('compaction region transaction', () => {
  ...
  it('bounds repeated multibyte checkpoint folds by the original message count', async () => {
    ...
    expect(session.snapshotEvents().filter(event => event.type === 'compaction/summary')).toHaveLength(2)
  })
  ...
  it('rechecks the durable lock before the final bounded transaction', async () => {
    ...
    expect(session.snapshotEvents().filter(event => event.seq > takeoverSeq)).toEqual([])
  })
  ...
  it('rechecks a closed turn inside nested bounded compaction', async () => {
    ...
    expect(compact.calls).toHaveLength(1)
    expect(session.surface.nodes).toEqual(surfaceAfterPrefix)
    expect(session.snapshotEvents().filter(event => event.seq > turnEndSeq)).toEqual([])
  })
  ...
  it('rejects a replacement turn before the next bounded transaction', async () => {
    ...
    expect(compact.calls).toHaveLength(1)
    expect(session.surface.nodes).toEqual(surfaceAfterPrefix)
    expect(session.snapshotEvents().filter(event => event.seq > replacementTurnSeq)).toEqual([])
  })
...
describe('automatic listener and loader composition', () => {
  ...
  function openRequestStep(session: Session): void {
    const turn = session.snapshotEvents().findLast(event => event.type === 'turn/start')?.data.turn ?? 1
    session.append('step/start', { turn, step: 1 })
  }
