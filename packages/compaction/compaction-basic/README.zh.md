...
HTTP 413 请求体拒绝会进入请求大小恢复，即使超大上下文路由的 token 压力仍低。只有达到或超过 `timeoutRecoveryBytes` 时，`TIMEOUT` 才进入同一路径；它会优先采用适配器提供的转换后估算以反映瞬态图片 offload，从而避免让未变化的大请求耗尽通用重试预算。只有恢复持久推进表层后，才会按 `learnedByteSafetyRatio` 从该估算学习更低的主动字节预算；该预算仅适用于对应 agent 与精确提供方／模型路由，语义上下文溢出不会改变它。
