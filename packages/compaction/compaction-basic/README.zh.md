...
本包让长时 agent 会话在接近模型上下文与网关请求体上限时仍能正常工作。token 或已配置字节压力上升时，它会自动把对话最旧的部分压缩为摘要，并保持近期部分完整；语义上下文溢出、HTTP 413 或符合条件的大请求超时发生后，它会压缩并重试。你也可以通过 `/compact` 按需压缩，并选择先修剪超大工具输出。压缩使用一次额外的模型请求，并且只保留该请求返回的摘要文本。它无法缩减系统提示词、工具或会话前缀，也无法拆分单个不可分单元（例如一次超大工具调用）。
