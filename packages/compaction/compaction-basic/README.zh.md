...
| `summarizationInputBytes` | 否（默认 `524288`） | 每个完整摘要请求（header、回放消息与固定指令）的上限。超大范围通过多个有界事务压缩；不可分请求超过上限时会在发送前失败。 |
