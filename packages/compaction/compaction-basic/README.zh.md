...
HTTP 413 请求体拒绝会进入请求大小恢复，即使超大上下文路由的 token 压力仍低。只有达到或超过 `timeoutRecoveryBytes` 时，`TIMEOUT` 才进入同一路径，从而避免让未变化的大请求耗尽通用重试预算。成功恢复会按 `learnedByteSafetyRatio` 从失败请求学习更低的主动字节预算；语义上下文溢出不会改变该预算。
