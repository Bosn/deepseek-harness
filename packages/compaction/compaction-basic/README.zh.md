...
`dsh-compaction-basic` 让长时 agent 会话在接近模型上下文与网关请求体上限时仍能正常工作。token 或已配置字节压力上升时，它会自动把对话最旧的部分压缩为摘要，并保持近期部分完整；语义上下文溢出、HTTP 413 或符合条件的大请求超时发生后，它会压缩并重试。你也可以通过 `dsh-command-compact` 的 `/compact` 按需压缩，并挂载 `dsh-compaction-tool-result-pruner` 先修剪超大工具输出。压缩的代价是一次额外的模型请求，它读取所选历史并写出摘要；只有摘要文本会被保留。它只压缩派生历史——无法缩减系统提示词、工具或会话前缀，也无法拆分单个不可分单元（例如一次超大工具调用）。
...
默认设置下会自动处理 token 压力；在语义溢出、HTTP 413 或足够大的超时请求后执行失败请求恢复；通过 `/compact` 按需压缩；以及——挂载修剪器时——在摘要前修剪超大工具输出。已确认的 413 会为精确路由学习更低的主动字节预算，语义溢出则不会。
...
| `maxOverflowRetries` | `1` | 语义溢出或请求大小恢复后的最大重试次数；`0` 只禁用恢复。 |
| `maxRequestBytes` | — | 已路由请求估算字节的可选主动上限；它也限制摘要请求，且必须容纳一条最小回放消息和固定指令。未设置时只使用 token 压力。 |
| `summarizationInputBytes` | `524,288` | 每条完整摘要请求的最大估算字节；策略继承后，它与 `maxRequestBytes` 的较小值必须容纳一条最小回放消息和固定指令。超大范围使用有界分层事务。 |
| `timeoutRecoveryBytes` | `524,288` | 使 `TIMEOUT` 有资格进入请求大小恢复的最小失败请求估算。 |
| `learnedByteSafetyRatio` | `0.75` | HTTP 413 或符合条件的超时恢复后，将失败请求估算用于学习主动预算的比例。 |
| `modelPolicies` | `[]` | 针对个别模型路由的精确 `{ provider, model, ...partialPolicy }` 覆盖。 |
| `auto` | `true` | 启用自动 token／字节压力与失败请求恢复；设为 `false` 则仅手动执行。 |

配置错误会快速失败：未知设置、重复的按模型覆盖、两种保留形式同时出现、无法容纳一条最小回放消息和固定指令的有效 `min(summarizationInputBytes, maxRequestBytes)` 上限、其他无效字节策略，或比例保留量不低于阈值，都会在加载时拒绝插件。顶层与按模型策略均在继承后采用相同校验。`modelPolicies` 中未指定的 token 与字节字段分别继承顶层策略。任何绝对 `retainTokens` 预算——顶层或按模型——不低于其阈值时，都会在该模型首次使用时失败，因为该比较需要模型的上下文大小。
...
当 `auto: true` 时，串行 `agent/pre-step` listener 会在请求派生前检查压力：它通过 `ctx.tokenMeter` 为最新持久路由请求 envelope 定价，当 token 压力或有效主动字节预算被越过时，先剪枝，再在保留已定价近期尾部的同时摘要最旧的平衡范围。`agent/request-error` listener 在失败请求所属 turn 与 step 仍开放时运行。它响应语义 `CONTEXT_WINDOW_EXCEEDED`、HTTP 413，以及转换后请求估算达到 `timeoutRecoveryBytes` 的 `TIMEOUT`；绕过常规压力与保留策略，尝试最大平衡头部缩减，并且只在表层替换 generation 前进后才授权重试。已确认的 413 与符合条件的超时恢复会学习更低的精确路由字节预算；取消全程保持最终决定权。

压力策略从拥有持久路由的适配器解析容量。适配器无法为有效动态路由返回容量时，手动压力路径会抛出目标特定配置错误；自动 listener 会对该精确目标警告一次，并携带完整历史继续。

### 摘要机制

直接 `ctx.llm.stream()` 调用使用已配置的提供方／模型对与上限，回退到最新已记录请求目标，然后再回退到 `AgentOptions` 对，而不运行仅用于 agent loop 的 `agent/request` 扩展点。该调用逐字回放会话自身的系统提示词、工具与已遮蔽区域消息——包括所选适配器必须解析或明确拒绝的图片引用——并将压缩指令作为最后一条 user 消息追加，从而复用提供方的热前缀 cache，而非使它失效。完整回放、工具、提示词与指令必须适合当前 `summarizationInputBytes` 上限；超大的平衡范围通过有界分层事务缩减，不可分的超限单元会在提供方分派前失败。调用将 `GenerateOptions.purpose` 设为 `compaction`；只有返回文本进入检查点，推理与工具调用都会被排除。图片输出会以 `UNSUPPORTED_CONTENT` 失败，而不是消失。替换 user 消息用 `<compacted-summary>` 标签框定摘要；原始摘要保留在 `compaction/summary` 事件上。

### 区域事务

事务验证表层范围与持久锁，追加 `compaction/start`，通过钩子生成摘要，重新验证稳定性（自动调用要求整个表层、手动调用只要求所选范围），拒绝未同时严格缩小精确 post-offload 回放路由 token 与估算字节的摘要，追加 `compaction/summary` 与替换 `user/message`，并恰好进行一次 `compaction/end` 尝试。已记录替换的影子价保持与路由无关，使重放投影具有确定性。活动的未匹配 start 是持久锁：位于较新 `session/end-seed` 之前的未匹配标记是先前生命周期留下的陈旧证据，不会阻塞；位于该边界之后的标记报告 `busy`。闭合失败会有意留下阻塞性的未匹配标记。完成清理与持久化后，取消仍具有最终决定权。
...
- **溢出分类由适配器维护**——提供方措辞可能改变；两个 DeepSeek 适配器会把可识别的语义与请求大小失败规范化为规范 code。只有超过 `timeoutRecoveryBytes` 的大请求超时才是保守的请求大小信号。
