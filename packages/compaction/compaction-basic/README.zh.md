...
- **不依赖模型的剪枝**：在压力或失败请求恢复符合条件后，可选的 [`ctx.toolResultPruner`](../compaction-tool-result-pruner/README.zh.md) 服务会在选择范围之前改写超大工具结果。Compact-basic 通过 `ctx.tokenMeter` 重新测量；如果压力已回到安全范围，就跳过摘要，否则对已剪枝的表层进行摘要。低于压力的步骤检查绝不剪枝。
...
- **收敛**：最多按 `compactionRetries` 重试头部检查点压缩；拒绝不能缩小源内容的摘要，如果重试仍无法回到阈值以下，则抛出异常。有上限的事务要求框定后的检查点能装入下一次带相同请求头与指令的请求，并且能缩小持久选中内容的字节数。未发生图片 offload 时，token 缩减与 post-offload 字节缩减都仍为强制要求。从已 offload 图片投影生成的最终检查点，可以用持久 pre-offload 字节缩减代替失真的图片元数据 token 比较；但会被后续有界事务再次消费的前缀检查点，仍必须严格缩小其 post-offload 消息。一次分层操作最多允许每个入口所选表层节点触发一次 summarizer 调用，因此只反复折叠检查点的路径也会有界终止。
- **摘要**：直接 `llm/stream` 调用使用已配置的提供方／模型对与上限，回退到最新已记录请求目标，然后再回退到 agent（智能体）目标，而不运行仅用于 agent loop 的 `agent/request` 扩展点。该调用会回放会话自身的系统提示词、工具与已遮蔽区域消息，并将压缩指令作为最后一条 user 消息追加，从而复用提供方的热前缀 cache，而非使它失效。自动压力、显式范围与 `compactNow()` 入口都会强制执行已路由的 `summarizationInputBytes` 上限。对于有上限的请求，系统会先检查完整的请求头、消息与指令估算；如果超限，则按顺序逐张把最旧图片引用替换为共享的省略占位符，直到完整请求可容纳，再在必要时通过独立的有界事务压缩较旧的平衡消息范围。每个新检查点都会进入下一次有界事务，因此各分区会收敛为一个替换，而不会在重建表层上累积。每条被移除消息都会进入摘要，不可分且仍超大的请求会在发送前失败。所选适配器必须解析或明确拒绝保留下来的图片。它将 `GenerateOptions.purpose` 设为 `compaction`，适配器可将其作为请求归因转发（DeepSeek 适配器发送 `x-deepseek-harness-compact: 1`），但不会触碰模型可见的请求体。只有返回的文本会进入检查点；推理（reasoning）和工具调用都会被排除，以免泄露私有推理或产生遗留调用；图片输出会以 `UNSUPPORTED_CONTENT` 失败，而不是消失。
- **框定**：替换 user 消息使用 `<compacted-summary>` 标签标记已建立的检查点上下文。原始摘要保留在 `compaction/summary` 事件上，后续自动周期会合并之前的检查点。
- **生命周期**：所有入口点都使用先记录标记的区域事务；超大范围会形成分层的有界事务。在每个事务同步追加 `compaction/start` 紧前，引擎都会重新读取持久锁与轮次状态，并要求分层操作开始时捕获的数字或 `null` 归属保持不变；检查与追加之间没有 `await`。因此，竞争的 `compaction/start`、轮次关闭或轮次被替换，都会在产生新的开始标记、摘要或替换之前失败。事务打开后会等待摘要，重新验证，再追加 `compaction/summary` 和替换，最后恰好进行一次闭合尝试。自动调用和显式范围调用要求数字标识的开放轮次归属，并要求整个表层保持稳定；串行 `agent/pre-step` listener 会在派生请求之前检查压力，而语义上下文溢出、HTTP 413 与已配置的大请求 `TIMEOUT` 恢复则经由 `agent/request-error` 进入，并且只在表层取得持久进展且通过下述失败请求准备状态检查后才允许重试。`compactNow()` 会预留空闲接纳，使用 `turn: null`，允许所选 span 之外追加仅追加上下文，flush 每次已闭合尝试，并在 `finally` 中释放接纳预留。
- **失败请求恢复**：语义上下文溢出不需要容量元数据。HTTP 413 始终选择请求大小恢复；只有适配器转换后的请求估算（缺省时使用表层请求估算）达到 `timeoutRecoveryBytes`，`TIMEOUT` 才选择该路径。两者都会绕过常规压力与保留，执行剪枝，再尝试一次最大平衡头部缩减，并留下最新不可分单元。完整替换或部分持久进展之后，只有原 request-error 的轮次与步骤仍开放、且没有活动持久压缩锁时，才会允许重试。所抛的 `busy` 失败还会额外阻止使用部分进展重试；其他未通过准备状态检查的情况会保留原始提供方失败，避免与竞争摘要并发。
- **失败处理**：活动的未匹配 `compaction/start` 是持久锁。位于较新 `session/end-seed` 之前的未匹配标记，是先前生命周期留下的陈旧证据，不会阻塞；位于该边界之后的标记报告 `busy`。摘要和 span 变更失败会以错误闭合，并保持各自所选 span 不变，但较早的有界前缀可能已经提交，且日志中会保留每次尝试。闭合失败会有意留下阻塞性的未匹配标记。压力检查中的运行故障会发出警告并继续；失败请求恢复会先应用上述请求归属检查，再决定能否从任何持久进展重试。完成清理与持久化后，取消仍具有最终决定权。
...
| `maxOverflowRetries` | 否（默认 `1`） | 语义上下文溢出或请求大小恢复后的最大重试次数；`0` 只禁用失败请求恢复。 |
| `maxRequestBytes` | 否 | 已路由模型请求估算 wire 字节数的可选上限。网关可能对 token 压力仍低的超大上下文请求返回 HTTP 413；配置后，压力压缩也会在该字节上限触发。 |
| `summarizationInputBytes` | 否（默认 `524288`） | 每个完整摘要请求（header、回放消息与固定指令）的上限。超大范围通过有界事务压缩，每次都会将先前检查点并入下一轮并收敛为一个替换；不可分请求超过上限时会在发送前失败。 |
| `timeoutRecoveryBytes` | 否（默认 `524288`） | `TIMEOUT` 可在通用重试前使用请求大小压缩的最小估算请求字节数。 |
| `learnedByteSafetyRatio` | 否（默认 `0.75`） | HTTP 413 或已配置的大请求超时恢复后，用于把失败请求估算字节转换为主动字节预算的顶层比例；必须位于 `(0, 1)`。 |
| `modelPolicies` | 否（默认 `[]`） | 精确的 `{ provider, model, ...partialPolicy }` 覆盖；匹配使用两个字段，不依赖 `listModels()`。 |
| `auto` | 否（默认 `true`） | 注册步骤边界压力与失败请求恢复 listener。设为 `false` 则仅手动执行。 |

每个 `modelPolicies` 配置项都接受上述策略字段，但不接受 `learnedByteSafetyRatio`、`auto` 和 `modelPolicies` 自身。如果配置项提供任意一个保留字段，就替换默认策略的保留选择；否则继承保留设置。摘要提供方／模型在每个配置项内仍然成对。

适配器可能无法为有效动态路由返回容量，已解析容量也可能暴露无效的绝对保留预算。此时手动压力检查会抛出目标特定配置错误；自动 listener 会对该精确目标警告一次，并携带完整历史继续。不相关的操作性失败仍会独立可见。失败请求恢复仍可尝试，因为提供方失败本身已经确立压缩的必要性。

HTTP 413 请求体拒绝会进入请求大小恢复，即使超大上下文路由的 token 压力仍低。只有达到或超过 `timeoutRecoveryBytes` 时，`TIMEOUT` 才进入同一路径；它会优先采用适配器提供的转换后估算以反映瞬态图片 offload，从而避免让未变化的大请求耗尽通用重试预算。从已确认 413 得出的每个正数预算都会约束当次恢复的 summarizer，包括低于 64 KiB 的值；低于该实用 floor 的预算不会保留给后续主动压力。只有恢复持久推进表层后，才会按 `learnedByteSafetyRatio` 从该估算学习更低的主动字节预算；保留的预算仅适用于对应 agent 与精确提供方／模型路由，语义上下文溢出不会改变它。
...
如果没有上限驱动的图片 offload，已回放系统提示词、工具与已遮蔽区域消息会与会话最后一个已路由请求逐字匹配，因此提供方的热前缀 cache 可复用至尾随指令之前；只有该指令与摘要输出未缓存。为满足完整请求上限而替换最旧图片时，复用会在该消息处结束。将摘要器路由到不同提供方／模型，或压缩非头部范围，同样会放弃相应复用。
