...
- **失败请求恢复**：语义上下文溢出不需要容量元数据。HTTP 413 始终选择请求大小恢复；只有估算请求达到 `timeoutRecoveryBytes` 时，`TIMEOUT` 才选择该路径。两者都会绕过常规压力与保留，执行剪枝，再尝试一次最大平衡头部缩减，并留下最新不可分单元。只要 `surface.replaceGeneration` 前进，就允许重试，包括剪枝在后续摘要工作抛出异常前已落地的情况。如果没有替换、目标特定上限已耗尽、已取消，或遇到无关错误，则保留原始提供方失败。
...
HTTP 413 请求体拒绝会进入请求大小恢复，即使超大上下文路由的 token 压力仍低。只有达到或超过 `timeoutRecoveryBytes` 时，`TIMEOUT` 才进入同一路径，从而避免让未变化的大请求耗尽通用重试预算。成功恢复会按 `learnedByteSafetyRatio` 从失败请求学习更低的主动字节预算；该预算仅适用于对应 agent 与精确提供方／模型路由，语义上下文溢出不会改变它。
