...
An HTTP 413 request-body rejection enters request-size recovery even on a mega-context route whose token pressure is low. A `TIMEOUT` enters the same path only at or above `timeoutRecoveryBytes`, preferring an adapter-supplied post-conversion estimate so transient image offload is reflected, and preventing a large unchanged envelope from consuming the generic retry budget. Recovery that durably advances the surface learns a lower proactive byte budget from that estimate using `learnedByteSafetyRatio`; the budget is isolated to that agent and exact provider/model route, and semantic context overflow does not alter it.
