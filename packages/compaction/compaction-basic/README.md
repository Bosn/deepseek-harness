...
An HTTP 413 request-body rejection enters request-size recovery even on a mega-context route whose token pressure is low. A `TIMEOUT` enters the same path only at or above `timeoutRecoveryBytes`, preventing a large unchanged envelope from consuming the generic retry budget. Successful recovery learns a lower proactive byte budget from the failed request using `learnedByteSafetyRatio`; the budget is isolated to that agent and exact provider/model route, and semantic context overflow does not alter it.
