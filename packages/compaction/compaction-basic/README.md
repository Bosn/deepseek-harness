...
| `summarizationInputBytes` | no (default `524288`) | Cap on each complete summarizer request: header, replay messages, and fixed instruction. Oversized ranges compact through multiple bounded transactions; an indivisible request over the cap fails before provider dispatch. |
