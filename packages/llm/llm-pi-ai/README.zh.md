...
每个 profile 都可以设置 `retryPolicy`；省略时使用 normal mode、最多重试五次，把 `TIMEOUT` 限制为一次重试，并且只对 `RATE_LIMIT` 使用一／三／五分钟冷却。`quotaWorded429IsRateLimit` 在内建 `qwen-token-plan` 路由上默认为 true，其他路由默认为 false，因此 token-plan 网关会重试带 quota 措辞的 HTTP 429，而 OpenAI 账户额度耗尽仍是终态 `QUOTA`。`apiKeyEnv` 是按请求经 harness 凭据 seam 解析的凭据引用，因此配置文件绝不包含密钥；解析为空的引用会让请求以 `MISSING_CREDENTIAL` 失败。省略它会让路由保持已配置但无密钥（configured-but-keyless）状态，对已安装目录路由而言即交由 pi-ai 提供方原生的环境发现。
...
pi-ai 不提供的路由需要 `api`、`baseURL` 与非空 `models` 列表；无法服务的 profile 会在写入处被拒绝，并点名路由与模型。失败携带稳定 code：无法使用的凭据以 `INVALID_CREDENTIAL` 失败并点名路由与引用，`apiKeyEnv` 引用解析为空的路由以 `MISSING_CREDENTIAL` 失败，未配置模型以 `UNKNOWN_MODEL` 失败。显式 HTTP 413 或严格识别的展平请求体上限措辞会映射为 `CONTEXT_WINDOW_EXCEEDED` 并携带 `status: 413`；语义上下文溢出使用同一 code，但不会虚构状态码。quota 措辞映射为终态 `QUOTA`，除非 HTTP 429 路由显式选择瞬态语义；网关包装已接受模型服务失败的消息映射为可重试 `SERVER`；响应侧审核映射为可重试 `CONTENT_FILTERED`；成功停止但不含内容时映射为可重试 `EMPTY_RESPONSE`。调用方取消仍为 `ABORTED`。`GenerateOptions.stop` 以 `UNSUPPORTED_OPTION` 被拒绝，因为 pi-ai 的通用流式 UI 无法跨提供方保证它。
...
每次操作都会在首次 await 前捕获已解析路由、重试策略、空闲超时、quota 措辞 429 策略与模型集合。适配器禁用 pi-ai SDK 重试，因此一次 `stream()` 只发一条提供方请求；持久步骤重试仍通过 `dsh-llm-retry` 可见。空闲到期只会把该请求中止为 `TIMEOUT`，更早发生的调用方取消仍为 `ABORTED`。
...
- **`maxRequestImageBytes` 只计算 base64 图片载荷**——文本、工具、描述符与 JSON 结构在该上限之外，因此它必须留有余量地低于网关请求体上限。卸载是确定性请求投影，不会记录为会话事件。失败会携带转换后 pi-ai context 的 UTF-8 字节估算，使请求大小与超时恢复不会计入已替换为占位文本的图片；精确协议序列化与请求标头不在该估算内。
...
- **提供方 HTTP 状态为尽力提取**——pi-ai 错误事件不跨提供方暴露稳定结构化状态；适配器会保留 SDK 展平消息中可识别的显式状态，否则保持缺省。
- **重试策略由提供方自有，而非 SDK 重试**——pi-ai SDK 重试保持禁用，因此持久 agent 步骤与 `llm/retry` 事件拥有每个可见尝试，直接 `ctx.llm.stream()` 调用仍是单次尝试。每条路由可覆盖共享有界策略，包括其 `RATE_LIMIT` 冷却调度。
