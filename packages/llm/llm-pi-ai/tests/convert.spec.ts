...
import { createToolResultMessage, createUserMessage, ToolCallId, CONTENT_FILTERED_CODE, CONTEXT_WINDOW_EXCEEDED_CODE, EMPTY_RESPONSE_CODE, createMessage } from '@deepseek-ai/dsh-llm'
...
describe('mapStopReason / mapUsage', () => {
  ...
  it('maps routable HTTP-ish error messages to stable codes', () => {
    ...
    expect(mapStopReason(assistant({ stopReason: 'error', errorMessage: 'HTTP 429: insufficient_quota' })))
      .toMatchObject({ kind: 'error', failure: { code: 'QUOTA', status: 429 } })
    expect(mapStopReason(
      assistant({ stopReason: 'error', errorMessage: 'HTTP 429: insufficient_quota' }),
      undefined,
      undefined,
      true,
    ...
    expect(mapStopReason(
      assistant({ stopReason: 'error', errorMessage: 'HTTP 402: insufficient_quota' }),
      undefined,
      undefined,
      true,
    )).toMatchObject({ kind: 'error', failure: { code: 'QUOTA', status: 402 } })
    ...
    expect(mapStopReason(assistant({
      stopReason: 'error',
      errorMessage: 'request too large for model context',
    }))).toEqual({
      kind: 'error',
      failure: {
