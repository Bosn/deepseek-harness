...
Supported profile fields are `apiKeyEnv`, `displayName`, `api`, `baseURL`, `models`, `modelOverrides`, `compat`, `defaultContextWindow`, `defaultMaxTokens`, `defaultInput`, `headers`, `reasoning`, `thinkingBudgets`, `cacheRetention`, `transport`, `timeoutMs`, `websocketConnectTimeoutMs`, `streamIdleTimeoutMs`, `quotaWorded429IsRateLimit`, `maxRequestImageBytes`, `requestImagePixelBudget`, `requestImageMaxBytes`, and `retryPolicy`. Each resolved profile retry policy is captured with that provider route; omission uses the shared bounded normal default of five retries with `TIMEOUT` capped at one retry. The stream-idle interval is a positive finite Node timer delay, defaults to five minutes, and covers only an outstanding provider read, not consumer think time. `quotaWorded429IsRateLimit` selects whether an explicit HTTP 429 whose body identifies exhausted quota is transient `RATE_LIMIT` instead of terminal `QUOTA`; it defaults to true for the built-in `qwen-token-plan` and `qwen-token-plan-cn` routes and false for every other route, so a custom Model Studio gateway opts in explicitly while OpenAI account exhaustion ends immediately. Every image route derives a deterministic request version from the provider-independent normalized attachment under `requestImagePixelBudget` (default 2048 by 2048 total pixels) and `requestImageMaxBytes` (default 1MiB raw bytes). Before reading attachments, `maxRequestImageBytes` applies to conservative request-version upper bounds and replaces the oldest over-budget images with fixed text; exact base64 lengths are checked again after retained versions are generated. The 20MiB default can retain fifteen maximum-size 1MiB versions after base64 expansion while leaving request-body headroom. The same version feeds inline base64, and its stable descriptor exposes the attachment id and actual request-image dimensions. Harness app attribution wins a conflicting configured header name.

The adapter forces pi-ai's SDK `maxRetries` to zero so one `stream()` call makes one provider request. The removed profile fields `maxRetries` and `maxRetryDelayMs` fail load instead of silently multiplying or hiding the separately composed agent-level retry budget. Idle expiry aborts only that SDK request and surfaces `TIMEOUT`; an earlier caller abort remains `ABORTED`. A concurrent request on the same adapter keeps progressing, and default generic recovery repeats an idle timeout once rather than consuming the shared five-retry budget.
...
- pi-ai reports failures as in-stream error events; these map to `finish {kind:'error'|'aborted', failure}` chunks. Plain HTTP 429 and rate-limit wording map to `RATE_LIMIT`. Quota wording maps to terminal `QUOTA` unless the response is an explicit HTTP 429 and the route resolves `quotaWorded429IsRateLimit: true`. Explicit HTTP status text is preserved as `failure.status`, request-body rejection maps to `CONTEXT_WINDOW_EXCEEDED` for request-size compaction, and text or usage evaluated against the resolved model's context window maps semantic overflow to the same canonical code. A gateway statement wrapping an upstream model-serving failure (an `error occurred in model serving` preamble that arrives mid-stream, after the gateway already accepted and streamed the request) maps to retryable `SERVER` instead of `INVALID_REQUEST`; `INVALID_REQUEST` is reserved for up-front request-shape rejections. Response moderation rejections — a wire `content_filter` finish reason or safety-gate wording naming the rejected output such as dashscope-intl's `Output data may contain inappropriate content.` — map to retryable `CONTENT_FILTERED`, even when the gateway phrases the rejection as an HTTP 400; request-side moderation (a prompt rejected before generation) carries no response-specific wording and stays `INVALID_REQUEST`. A terminal `stop` whose message carries no content blocks maps to a `finish {kind:'error'}` with code `EMPTY_RESPONSE` (retried by default policy) instead of a successful empty message.
...
- **`maxRequestImageBytes` counts base64 image payload only** — text, tools, and JSON structure ride outside the bound, so it must sit below the gateway's request-body cap with headroom. Offload is decided at request conversion as a pure function of history and configuration and is not recorded as a session event. Failures carry a UTF-8 byte estimate of the converted pi-ai context so timeout recovery does not price images already replaced by placeholders; exact protocol serialization and request headers remain outside that estimate. Per-route capability metadata (image count, per-image size, total request size) driving admission and assembly together is deferred design work.
...
- **Provider HTTP status is best-effort from flattened error text** — pi-ai error events do not expose a stable structured HTTP status across providers. The adapter records an explicit status recognized in the SDK's flattened message and otherwise leaves it absent; stable harness error codes remain authoritative. Non-2xx response headers are unavailable.
