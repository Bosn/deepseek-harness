...
- **`maxRequestImageBytes` counts base64 image payload only** — text, tools, and JSON structure ride outside the bound, so it must sit below the gateway's request-body cap with headroom. Offload is decided at request conversion as a pure function of history and configuration and is not recorded as a session event. Failures carry a UTF-8 byte estimate of the converted pi-ai context so timeout recovery does not price images already replaced by placeholders; exact protocol serialization and request headers remain outside that estimate. Per-route capability metadata driving admission and assembly together is deferred design work.
