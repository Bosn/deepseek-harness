...
提供方适配器还会提供由 `dsh-llm-retry` 执行的解析后路由策略。省略提供方配置时使用有界 normal mode：共享五次重试、`TIMEOUT` 最多重试一次，并为 `RATE_LIMIT` 采用一、三、五分钟冷却调度。本服务把有效策略捕获在最终适配器边界上，但从不自行执行重试。

当消费方必须满足精确的完整请求字节上限时，`offloadRequestImagesUntil(messages, fits)` 会先把原始消息交给纯判定函数，再逐张以 `offloadedImageText()` 替换最旧图片并重新检查。它保留持久消息；如果只移除图片仍无法满足限制，则返回最终的无图片投影。
