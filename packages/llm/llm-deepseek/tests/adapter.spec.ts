...
describe('DeepSeekAdapter against a mock server', () => {
  ...
  it('reports exact post-offload request bytes for a Vision HTTP 413', async () => {
    ...
    const adapter = adapterOf({
      ...
      maxRequestFilesBytes: 3,
      imageOffloadByteQuantum: 3,
      models: [{ id: 'vision', inputModalities: ['text', 'image'] }],
    }, attachments)
  ...
  it('omits request bytes when image serialization times out before a payload exists', async () => {
    vi.useFakeTimers()
    const attachments = {
      readImageRequest: vi.fn((_ref: ImageAttachmentRef, _policy: ImageRequestPolicy, signal?: AbortSignal) =>
        new Promise<RequestImageAttachment>((_resolve, reject) => {
