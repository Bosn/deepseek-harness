...
export interface Config {
  ...
  /** Fork alias: maximum accumulated base64 image payload per request (default 20 MiB); overrides `maxInlineRequestImageBytes`. */
...
export function resolveAdapterOptions(config: Config, environment?: LaunchEnvironmentSnapshot): ResolvedDeepSeekOptions {
  ...
  let maxInlineRequestImageBytes = config.maxInlineRequestImageBytes ?? DEFAULT_MAX_INLINE_REQUEST_IMAGE_BYTES
  ...
  // Fork alias: maxRequestImageBytes keeps its own validation message and,
  // when set, takes over the inline fallback budget.
  if (config.maxRequestImageBytes !== undefined) {
    if (!Number.isSafeInteger(config.maxRequestImageBytes) || config.maxRequestImageBytes <= 0) {
      throw new Error('llm-deepseek: maxRequestImageBytes must be a positive safe integer')
    }
    maxInlineRequestImageBytes = config.maxRequestImageBytes
  }
