 ...
 *
 * Every HTTP 429 classifies as `RATE_LIMIT`, including quota-worded 429s
 * (`insufficient_quota` and alike): gateways such as qwen Model Studio emit
...
export class DeepSeekAdapter extends LlmAdapter {
  ...
  private async * request(
    ...
    while (true) {
      ...
      // Prepared per attempt so the TRANSPORT label above covers exactly the
      // transport boundary, never a serialization failure; the callback feeds
      // the fork's request-byte-budget recovery instrumentation.
