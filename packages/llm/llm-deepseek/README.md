...
- **DashScope Intl continuation rule**: the canonical `https://dashscope-intl.aliyuncs.com/compatible-mode/v1` base URL treats an empty or null streamed tool-call `id` or `name` as omitted, preserving the established non-empty identity while argument fragments continue to concatenate. Other endpoints keep the existing translator semantics, including replacement when a field is present.
