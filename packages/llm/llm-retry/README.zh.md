...
`RATE_LIMIT` 失败不使用快速指数退避，而是依次等待冷却调度：`backoff.rateLimitDelaysMs` 为每次尝试列出一个等待时长（默认 `[60000, 180000, 300000]`），因此默认行为是三次分别等待一、三、五分钟的冷却重试，让网关 429 限流——包括 qwen Model Studio `insufficient_quota` 这类带 quota 措辞的 429——有时间恢复。该调度就是 `RATE_LIMIT` 的尝试预算：normal mode 下有效上限为 `min(maxRetries, 调度长度)`；空数组关闭该调度、退回指数退避；always mode 在调度耗尽后继续使用指数退避。有效的提供方 `Retry-After` 只会抬高冷却等待——调度项是下限——超出定时器范围的值则被忽略、改用调度项。
