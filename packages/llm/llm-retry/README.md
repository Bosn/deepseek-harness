...
RATE_LIMIT failures wait out a cooldown schedule instead of the fast exponential backoff: `backoff.rateLimitDelaysMs` lists one wait per attempt (default `[60000, 180000, 300000]`), so the default delivers three one-, three-, and five-minute cooldown retries while gateway 429 throttling — including quota-worded 429s such as qwen Model Studio's `insufficient_quota` — clears. The schedule is the RATE_LIMIT attempt budget: in normal mode the effective cap is `min(maxRetries, schedule length)`, an empty array disables the schedule and falls back to exponential backoff, and always mode continues with exponential backoff after the schedule is consumed. A valid provider `Retry-After` raises a cooldown wait — the scheduled entry is the floor — while an out-of-timer-range value is ignored in favor of the entry.
