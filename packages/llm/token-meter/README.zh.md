...
该包还导出供请求大小策略使用的纯字节估算器。它们会保守计价内置消息与内容块的 JSON 序列化，包括字符串转义和字段 framing；图片则在持久引用的序列化开销之外加上 base64 载荷。适配器特定的协议转换仍可能不同，因此可用时应以适配器提供的转换后估算为更权威依据。
...
`ctx.tokenMeter` 暴露三个操作。`measure(session, requestHeader?)` 在同一个已消费日志 revision 上返回独立、深度不可变的快照：`totalTokens` 是请求与响应压力，`surfaceTokens` 是仅表面的路由定价总量，等于 `nodes[].tokens` 之和。可选 `requestHeader` 覆盖会选择计价路由与压力字段；节点集合仍描述当前会话。`estimateMessage(message)` 用固定启发式规则为一条消息计价。`estimateMessages(messages, requestHeader?)` 会按所选路由为独立消息序列计价，包括适配器拥有的图片价格，使临时请求投影与持久测量使用同一定价词汇。每次 `measure()` 调用都会克隆带位置的表面节点，因此测量是 O(surface)。
...
- **固定启发式规则是近似值**——没有可复用提供方用量的文本按字符数加结构开销计价，而非精确提供方分词器；只有声明了定价的路由上的图片出现处携带提供方精确的视觉 token。字节压力使用保守 JSON 代理，而不是适配器的精确协议 serializer。
