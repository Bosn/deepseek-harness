...
describe('route-aware image pricing', () => {
  ...
  it('prices detached messages with the same routed image policy', async () => {
    const { meter } = await harness(() => fixedPricing)
    const messages = [imageMessage('first'), imageMessage('second')]

    expect(meter.estimateMessages(messages, header('vision')))
      .toBe(messages.reduce((total, message) => total + routedMessageTokens(message), 0))
    expect(meter.estimateMessages(messages))
      .toBe(messages.reduce((total, message) => total + estimateMessage(message), 0))
  })
