...
import { analyzeNode, commitSurfaceTokens, planSurfaceTokens } from './surface-fold.ts'
...
export class TokenMeter extends Service {
  ...
  /**
   * Price a detached message sequence under one routed request envelope.
   * Transient projections such as image offload use this operation so their
   * route-token comparisons do not depend on durable surface nodes.
   * @param messages - ordered model-visible messages to price without mutation.
   * @param requestHeader - optional envelope selecting route-owned image pricing.
  ...
  estimateMessages(messages: readonly Message[], requestHeader?: EpochHeader): number {
    ...
    const nodes = messages.map((message, index) => analyzeNode(SessionSeq(index), message))
