...
/**
 * Build the replay fold's image-aware facts for one message.
 * @param seq - positional identity retained by surface measurements.
 * @param message - message to analyze, or `null` for a non-projecting event.
 * @returns fixed and image-free prices plus ordered image occurrences.
 */
export function analyzeMessage(seq: SessionSeq, message: Message | null): MeterSurfaceNode {
...
export function planSurfaceTokens(
  ...
  const node = analyzeMessage(event.seq, deriveEventMessage(event))
