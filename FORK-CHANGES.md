...
| 2026-09-10 | [#28](https://github.com/Bosn/deepseek-harness/pull/28) | repo process | Raise the `AGENTS.md` `verify-doc-budgets` ceiling 2200 → 2400 words (upstream keeps 1950): the merged file carries upstream's authoritative text plus the fork process rules that upstream has no counterpart for, and relocation or condensation would drop required rules. |
| 2026-09-10 | [#29](https://github.com/Bosn/deepseek-harness/pull/29) | `ui-chat`, `ui-trajectory` | Restore per-record first-token timing: a settled Assistant record derives TTFT from the compact stream embedded in `assistant/message` through the host projection's early-exit reader, so the Trajectory detail panel, the timeline TTFT/decoding split, and the Chat Turn time dialog report latency again without resuming per-delta replay. |
| 2026-09-11 | [#31](https://github.com/Bosn/deepseek-harness/pull/31) | `turn-notify-wechat` | Persist admitted completion notices, retry definite send failures within a fixed budget, and resume queued delivery after reload while preserving ambiguous sends without replay. |
