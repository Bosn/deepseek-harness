...
const turnNotifyWechatSender = fileURLToPath(new URL(
  '../../apps/cli/tests/profiles/sdk/fixtures/turn-notify-wechat-sender.mjs',
  import.meta.url,
))
...
interface SdkAssertions {
  ...
  /** Workspace file that must exist before the runtime may shut down. */
  workspaceReadyFile?: string
}

const SDK_ASSERTIONS: Readonly<Record<string, SdkAssertions>> = {
  'session-title-after-turn': {
    workspaceReadyFile: 'wechat-notice.txt',
  },
...
const workspaceSetups: Readonly<Record<string, (cwd: string, runtimeRoot: string) => Promise<void>>> = {
  async 'turn-notify-wechat'(_cwd, runtimeRoot) {
    const command = join(runtimeRoot, 'turn-notify-wechat-sender.mjs')
    await cp(turnNotifyWechatSender, command)
    await chmod(command, 0o700)
    await writeFile(join(runtimeRoot, 'turn-notify-wechat-route.env'), [
...
async function seedWorkspace(scenario: CorpusScenario, cwd: string, runtimeRoot: string): Promise<void> {
  ...
  await prepare(cwd, runtimeRoot)
}

async function waitForWorkspaceFile(cwd: string, relativePath: string): Promise<void> {
  const path = join(cwd, relativePath)
  const deadline = Date.now() + 10_000
  while (!existsSync(path)) {
    if (Date.now() >= deadline) throw new Error(`SDK snapshot workspace file did not appear: ${relativePath}`)
...
async function runScenario(scenario: CorpusScenario): Promise<{
  ...
  await seedWorkspace(scenario, cwd, patchRoot)
  ...
  try {
    ...
    try {
      ...
      if (assertions.workspaceReadyFile !== undefined) {
        await waitForWorkspaceFile(cwd, assertions.workspaceReadyFile)
      }
    } finally {
      subscription.close()
    }
