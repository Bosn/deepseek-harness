...
- A claimed turn passes through pre-step entry and zero or more request steps. A handled model-request failure rebuilds the request within the same open turn and step. Durable `step/end` and `turn/end` events are appended only when those scopes ultimately close.
...
A model-request failure enters `agent/request-error` with the normalized `LlmFailure`, optional resolved retry policy, and live turn signal while its step and turn remain open. A listener that owns recovery repairs state, returns `{ kind: 'retry' }`, and stops delegating. After rechecking the signal, the loop rebuilds the request inside the same `step()` invocation; no `step/end` or `turn/end` event separates request attempts. An unhandled failure propagates through the open boundaries, `agent/error` remains the live error notification for consumers that report failures independently of durable turn settlement, and both boundaries close exactly once. The [retry-action decision](2026-07-27-request-error-retry-action.md) supersedes the command-shaped part of this design.
...
The observable machine is smaller and compositional: registration lifetime, activity, item progress, and durable turn settlement can be followed independently. In particular, a `turn/end` event does not imply `agent.status === 'idle'`; it records one turn, while `agent/status` reports whether the whole agent is active.

Plugins no longer rewrite every phase of the loop. There is no request-only message prefix, assistant-message transform, post-step checkpoint, generic continuation enum, generic terminal-stop result, or general live-agent retry command. Extensions use the remaining owned channels instead of recreating those phases.

Continuation plugins publish durable steering rather than returning an unlogged reason. Recovery plugins handle a failed request before its step closes and return an explicit retry action. Multiple request attempts share one durable turn and step, while asynchronous repair and policy ownership stay at one narrow waterfall boundary.
