...
- 已领取的轮次经过 pre-step 进入决策和零个或多个请求步骤。处理过的模型请求失败会在同一个仍打开的轮次与步骤内重建请求。只有这些作用域最终关闭时，才会追加持久的 `step/end` 与 `turn/end` 事件。
...
模型请求失败会在其步骤与轮次仍打开时进入 `agent/request-error`，并携带标准化 `LlmFailure`、可选的已解析重试策略和仍有效的轮次信号。负责恢复的监听器修复状态、返回 `{ kind: 'retry' }`，并停止继续委托。循环重新检查信号后，会在同一次 `step()` 调用内重建请求；各次请求尝试之间没有 `step/end` 或 `turn/end` 事件。未处理的失败会从仍打开的边界传播出去；对于需要脱离持久轮次结算单独报告失败的消费方，`agent/error` 仍作为实时错误通知保留，两个边界也都只关闭一次。[重试动作决策](2026-07-27-request-error-retry-action.zh.md)取代了本设计中命令形式的部分。
...
可观察状态机更小，也更容易组合：注册生命周期、活动状态、条目进度和持久轮次结算可以分别追踪。尤其是，`turn/end` 事件并不意味着 `agent.status === 'idle'`；前者记录一个轮次，`agent/status` 则报告整个 agent 是否处于活动状态。

插件不再能够改写循环的每个阶段。不再提供仅用于请求的消息前缀、助手消息变换、步骤后检查点、通用的继续执行枚举、通用的终止结果或通用的活跃 agent 重试命令。扩展改用剩余的归属明确的通道，而不是重新构造这些阶段。

负责继续执行的插件发布可持久化的 steering，而不是返回未记录到日志中的原因。恢复插件会在失败请求所属步骤关闭前处理错误，并返回显式重试动作。多次请求尝试共用一个持久轮次和步骤，同时异步修复与策略归属集中在一个狭窄的 waterfall 边界。

收件箱生命周期用于补充持久会话日志，而非取代它。`MessageId` 将接受操作与领取或丢弃操作关联起来；轮次编号与步骤编号、消息、工具活动和终止原因仍属于会话事实。

## 相关内容

- [统一 agent 交付路由，并将注入上下文合并到 user/message](../../archived/architecture/2026-07-22-unified-send-and-coalesced-user-messages.md)
