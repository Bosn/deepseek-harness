...
The failed step closes before recovery runs. A handling listener repairs durable state, returns `{ kind: 'retry' }`, and stops waterfall delegation. The loop then closes the failed turn and opens one retry turn from the durable log without an intervening idle notification. Retry policy and attempt counts remain plugin-owned; compaction-basic clears its per-agent recovery count when the chain reaches terminal `agent/settled`. DeepSeek and pi-ai normalize recognized context-limit and HTTP 413 failures to `CONTEXT_WINDOW_EXCEEDED`, preserving explicit status when available. The [retry-action decision](../simplification/2026-07-27-request-error-retry-action.md) owns the return boundary.
