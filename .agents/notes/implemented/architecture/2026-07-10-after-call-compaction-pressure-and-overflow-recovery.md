# Agent Note: After-call compaction pressure and failed-request recovery
...
The failed request remains inside the open step while recovery runs. A handling listener repairs durable state, returns `{ kind: 'retry' }`, and stops waterfall delegation. The loop then rebuilds and repeats the request inside the same open turn and step. Retry policy and attempt counts remain plugin-owned; compaction-basic clears its per-agent recovery count when a successful `assistant/message` starts a fresh sequence or `agent/status` reaches `idle`. DeepSeek and pi-ai normalize recognized context-limit and HTTP 413 failures to `CONTEXT_WINDOW_EXCEEDED`, preserving explicit status when available. The [retry-action decision](../simplification/2026-07-27-request-error-retry-action.md) owns the return boundary.
...
`CompactionEngine.compactIfNeeded(agent, trigger, signal)` accepts `trigger: 'pressure' | 'context-overflow' | 'request-size'`. The interface gains no estimation methods or token types; `ctx.tokenMeter` remains the reusable accounting owner.
...
For semantic context overflow and request-size recovery, compaction-basic requires no capacity metadata and bypasses scalar pressure and the normal retained-token budget. HTTP 413 always selects request-size recovery; `TIMEOUT` selects it only above the configured estimated-byte threshold. Recovery prunes first, then chooses the maximal tool-balanced head range while leaving the newest indivisible unit and attempts shrinking summary compaction under the same signal when a range exists. Request-size recovery caps each complete summarizer request and partitions large ranges only at balanced boundaries. The automatic listener snapshots `session.surface.replaceGeneration` and returns `{ kind: 'retry' }` whenever pruning or summarization increases it. This remains true when pruning lands before later summary work throws; cancellation still wins. A backend returning a result without replacement cannot authorize retry, while pruning-only progress can authorize a retry without a `CompactionResult`. The request-size heuristics and byte defaults are owned by the [request-size recovery decision](../bug-fix/2026-08-21-request-size-timeout-recovery.md).

`maxOverflowRetries` is optional and defaults to `1`; `0` disables semantic-overflow and request-size recovery without disabling pressure. `auto: false` registers neither automatic listener. Unrelated errors, exhausted attempts, an already-aborted signal, a missing routed model, no safe range, no generation change, and recovery throws before any replacement all delegate to the next listener. With no later recovery, the loop reports the original provider error object and code. A recovery throw after generation advances authorizes retry from durable progress; cancellation or disposal remains authoritative even if recovery work completes concurrently.
...
Unit tests cover the final-adapter normalization boundary, same-turn and same-step retry sequencing and reset, cancellation and disposal, step-boundary ordering, routed-envelope pressure, pressure-gated pruning, pruning-only relief, pruned-input summarization, balanced failed-request reduction, durable prune progress before later failure, generation proof, caps, delegation, and auxiliary-call routing. Real-loop tests cover thrown and in-band context overflow, HTTP 413, and eligible large-request TIMEOUT through pruning or summary compaction to a reconstructed retry request.
...
- **Close the step before request recovery** — rejected because provider attempts are internal to one model-request step. Keeping the step open gives scheduled retries, rebuilt requests, and the final outcome one durable turn and step identity.
