...
The shared transient-code set is intentionally small: adapter mappings for `RATE_LIMIT` and `SERVER`, explicit `TIMEOUT` and `TRANSPORT` codes for remote failures, `EMPTY_RESPONSE` for a completed provider response with no content blocks, and `CONTENT_FILTERED` for a response an upstream safety gate rejected on moderation grounds. Both adapters classify the empty case as an error finish; see [empty model responses are retryable](../bug-fix/2026-07-24-empty-model-response-is-retryable.md). The response-side moderation classification, its retry default, and its request-side `INVALID_REQUEST` boundary live in [content-filtered outputs are retryable](../bug-fix/2026-08-23-content-filtered-output-is-retryable.md). The direct DeepSeek adapter classifies every explicit HTTP 429 as `RATE_LIMIT`. Pi-ai does the same for plain 429 responses, but quota wording remains terminal `QUOTA` unless the explicit 429 arrives on a route configured for transient quota wording; the built-in qwen token-plan routes enable that behavior by default, while OpenAI and unknown routes do not. Quota wording on non-429 statuses remains terminal on every route. The [cooldown decision](../bug-fix/2026-08-21-rate-limit-cooldown-retry.md) owns this classification and the minute-scale waits. Authentication, invalid request, semantic context overflow, protocol, abort, and unknown failures keep distinct stable codes and are not transient by default. HTTP 413 uses the context-overflow code to select rebuilt-request compaction, as owned by the [request-size recovery decision](../bug-fix/2026-08-21-request-size-timeout-recovery.md). Adding a code requires adapter fixtures and a documented policy decision; it does not require expanding a second failure-class enum.
...
- Rewriting content a safety gate rejected, and repairing malformed tool arguments, refusals, or other semantic model output — retrying a rejected sample re-samples with the bounded budget, it never rewrites the blocked tokens.
...
- `agent/request-error` carries current failure facts, the failed call's provider and open turn/step, the serving registration's immutable retry policy, and the live turn signal; durable retry records supply policy history, and alternating transient/context-overflow integration tests prove the two policies consume only their own finite budgets.
- Each provider adapter validates its nested retry policy at Loader startup, and `ctx.llm` captures it with the route; normal mode delegates specialized recovery first and makes at most `min(maxRetries, maxRetriesByCode[code]) + 1` provider requests for a capped code when no other policy applies.
...
- Real agent-loop tests cover failure before chunks, partial chunks then failure, thrown and in-band failures, retry to success inside the same turn, exhaustion to structured `turn/end.reason`, and composition with `dsh-compaction-basic` failed-request recovery.
- The partial-chunk integration test proves failed chunks remain attributed to the shared open step, no assistant message or tool side effect is committed for that failed attempt, and the successful retry records its own chunk seqs and provider/model route before the step closes.
...
- Every retry attempt is visible inside its owning turn as the failed attempt's chunks plus `llm/retry`, and adapter-level single-attempt behavior prevents hidden SDK retries from multiplying policy decisions. A retry can still duplicate provider billing even when no chunk arrived; normal mode limits that risk, while explicit always mode accepts it until cancellation, success, or a downstream durable replacement suppresses fallback.
- Provider SDKs may hide status or retry headers. Pi-ai extracts only explicit status syntax from the SDK's flattened error and does not claim unavailable non-2xx header facts; adapters retain the stable facts they expose and otherwise use a coarse code, while recovery policy consumes structured `LlmFailure` rather than parsing provider text itself.
...
- [After-call compaction pressure and failed-request recovery](../../implemented/architecture/2026-07-10-after-call-compaction-pressure-and-overflow-recovery.md) owns the current open-step request-recovery extension point and bounded compaction retry.
...
- [RATE_LIMIT cooldown retry](../../implemented/bug-fix/2026-08-21-rate-limit-cooldown-retry.md) owns the one/three/five-minute cooldown schedule and route-aware quota-worded-429 classification.
