...
- `agent/request-error` 携带当前失败事实、失败调用的提供方与仍处于打开状态的轮次／步骤、实际服务注册所对应的不可变重试策略，以及仍有效的轮次 signal；持久重试记录提供策略历史，暂时性失败／上下文溢出交替发生的集成测试证明两种策略只消耗各自的有限预算。
...
- 真实 agent-loop 测试覆盖分片前失败、部分分片后失败、抛出及带内失败、在同一轮次内重试至成功、耗尽后写入结构化 `turn/end.reason`，以及与 `dsh-compaction-basic` 失败请求恢复的组合。
- 部分分片集成测试证明：失败分片仍归属于共用的开放步骤，该次失败尝试不会提交 assistant 消息或工具副作用，成功的重试会在步骤关闭前记录自己的分片 seq 和提供方／模型路由。
...
- 每次重试尝试都在其所属轮次内以失败尝试的分片加 `llm/retry` 的形式可见，适配器级的单次尝试行为会防止隐藏的 SDK 重试成倍增加策略决策。即使没有分片到达，重试仍可能造成提供方重复计费；normal mode 会限制此风险，而显式 always mode 会接受它，直至取消、成功或下游持久替换抑制回退。
- 提供方 SDK 可能隐藏状态或重试标头。pi-ai 只从 SDK 展平错误中提取显式状态语法，不会声称拥有实际不可用的非 2xx 响应头事实；适配器会保留 SDK 公开的稳定事实，否则使用粗粒度 code，恢复策略仍消费结构化 `LlmFailure`，不会自行解析提供方文本。
