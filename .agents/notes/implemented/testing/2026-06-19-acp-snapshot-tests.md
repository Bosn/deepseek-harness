...
### Two required outputs and optional complete workspace state: normalize, then compare

A snapshot run asserts two required normalized outputs because the harness's external APIs are distinct, and may assert complete generated workspace state that neither API carries:
...
3. An optional **complete workspace snapshot** captures every generated text file, binary file, symbolic link, and empty directory after graceful shutdown. A scenario declares `workspace.final: true` and commits the exact cwd-relative tree under `workspace.expected/`; otherwise any difference between initial and final workspace state fails. The expected tree is reviewed input and is not rewritten by record or refresh.

The required surfaces are complementary: stdout covers the minimal automation wire, while JSONL covers loop, tool, and boundary structure that the wire intentionally omits. A complete workspace snapshot pins filesystem state that neither required surface carries.
