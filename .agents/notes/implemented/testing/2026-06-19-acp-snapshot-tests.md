...
### Two required outputs and optional workspace artifacts: normalize, then compare

A snapshot run asserts two required normalized outputs because the harness's external APIs are distinct, and may assert generated workspace artifacts that neither API carries:
...
3. Optional **workspace output snapshots** capture complete generated-workspace text artifacts after graceful shutdown. Each artifact path is cwd-relative, and its expected output is a unique direct filename inside the scenario directory. Registration rejects traversal, duplicate names, and suite-owned fixture names before record or refresh can write; refresh writes back the complete captured text.

The required surfaces are complementary: stdout covers the minimal automation wire, while JSONL covers loop, tool, and boundary structure that the wire intentionally omits. A workspace output snapshot pins a generated artifact that neither required surface carries.
