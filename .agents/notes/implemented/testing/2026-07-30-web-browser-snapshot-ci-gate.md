...
Local `pnpm run test:web` continues to build first and then run the full browser suite; `test:web:built` is the entry point for existing build artifacts. Under the [explicit browser evidence policy](../process/2026-08-15-explicit-browser-gif-evidence.md), agents run `DSH_SNAPSHOT=refresh pnpm run test:web` only when the user explicitly requests browser acceptance and the changed contract is owned by a browser expected output. They review every expected-output diff and then verify in replay mode that no files are written. A user-visible change or pull request alone does not authorize either browser run.
