...
`host.openPath` is a unary RPC covered by the carrier's prefix-wide trust fence (same carrier as `host.pickDirectory`); since the [openPath-trust decision](../architecture/2026-08-25-openpath-rides-trusted-hosts.md) it is not loopback-pinned — loopback and every declared `trustedHosts` authority reach it, and undeclared authorities are refused. Platform adapters open without a shell: `open` on macOS, PowerShell `Invoke-Item` on Windows, and `xdg-open` on desktop Linux; browser-renderable documents prefer the named default browser on macOS and desktop Linux. WSL is a separate host shape despite Node reporting `linux`: the adapter recognizes its environment or Microsoft kernel release, translates the Linux path with `wslpath -w`, and passes the resulting Windows/UNC path to the same PowerShell handoff. The opener's platform facts and command runner are injectable for tests. URL-only read args (`web_fetch`) are not file links.
...
Clicking a file path in a tool row opens that path on the host. Non-file tool rows are inert summaries (expand toggles remain where the row already supported them). Undeclared remote authorities cannot invoke `host.openPath`; an operator-declared `trustedHosts` authority such as a Tailscale tailnet can ([openPath-trust](../architecture/2026-08-25-openpath-rides-trusted-hosts.md)). A Host or OS refusal is owned by the chat view: it shows the thrown reason and retries the same path ([file-open failure](../bug-fix/2026-08-18-tool-row-file-open-failure.md)).
