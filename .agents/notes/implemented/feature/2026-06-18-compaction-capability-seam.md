...
2. **Implementation** — `@deepseek-ai/dsh-compaction-basic`: a concrete `BasicCompactionEngine` that consumes `ctx.tokenMeter` and owns the tail→head retention walk, summarization via `ctx.llm.stream()`, the surface replacement, the lock, pre-step pressure, and failed-request recovery for semantic overflow and request size. `summarize()` is its sole subclass hook; pricing and replay stay with the meter.
...
Semantic context overflow and request-size failure take a separate path. `agent/request-error` receives the original request error while its turn and step remain open. Compact-basic owns its per-agent recovery count, prunes before forcing useful balanced reduction, and returns `{ kind: 'retry' }` only if `session.surface.replaceGeneration` increases, including pruning-only progress when no summary range exists. HTTP 413 always qualifies; a TIMEOUT qualifies only for a configured large request. Request-size recovery bounds complete summarizer requests and uses balanced hierarchical transactions when one range cannot fit. The loop then reconstructs and repeats the request inside that same turn and step. No replacement, a recovery failure before any replacement, cancellation, an exhausted cap, or an unrelated error preserves the original provider failure. If pruning already advanced the generation before later summary work fails, recovery retries from that durable pruned surface unless cancellation or disposal wins. The complete lifecycle decision is in the [after-call recovery Agent Note](../architecture/2026-07-10-after-call-compaction-pressure-and-overflow-recovery.md); byte heuristics and limits are in the [request-size recovery Agent Note](../bug-fix/2026-08-21-request-size-timeout-recovery.md).
...
provider overflow inside open turn/step
await waterfall agent/request-error  ⟵ forced compaction between attempts
retry → repeat request in same step   ⟵ derives from the replacement surface
```
...
- **Automatic extension points**: `agent/pre-step` (`@mode waterfall`) handles pressure before request derivation and `agent/request-error` (`@mode waterfall`) handles final request failures while their turn and step remain open. The pre-step payload carries the claimed batch, turn, step, and signal (see the [payload-object events decision](../architecture/2026-08-06-agent-event-payload-objects.md)), with no compaction-only prompt/prefix payload.
...
- **Loop:** Tests pin pre-step after the preceding `step/end` and before the next `step/start`, actual `agent/request` routing, same-turn and same-step retry numbering, and complete thrown/in-band overflow → compaction → reconstructed retry composition.
...
- **Snapshot:** The assembled HTTP 413 scenario derives the auxiliary call from `compaction/summary` only when `llmStreamCall: true` proves that the local LLM service consumed it; canonical reconstructed blocks pin the complete recovery without provider delta partitioning.
