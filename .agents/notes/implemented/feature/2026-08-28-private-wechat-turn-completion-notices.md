...
One pending or queued completion is retained per top-level session. After a configurable five-second settle delay, the plugin resolves the current `ctx.sessionTitle` title, bounds it, and sends `DSH任务 [<status>]：<title>\n<summary>`. A newer terminal turn immediately removes an older retained completion before deciding whether the newer turn has visible text, so a queued result cannot start during the newer settle window and a textless result stays silent. The settle delay and subprocess timeout cannot exceed Node's supported timer maximum. A missing title is a payload-free warning rather than an unidentifiable notification.

The plugin reads account and target from a deployment-owned constants file at load. It spawns the configured OpenClaw wrapper without a shell and with an allowlisted environment, validates a sent receipt carrying the configured channel and message id, and turns delivery failures into payload-free warnings. The idempotency key binds the session id and exact terminal turn, sequence, timestamp, and reason. Configurable limits bound live delivery subprocesses and all retained pending plus queued deliveries across sessions. A newer queued turn for the same session replaces its older queued notice; retention overflow drops the globally oldest pending or queued notice, retains the newest completion, and emits a payload-free warning. Disposal and retry durability follow the [outbox decision](../bug-fix/2026-09-11-durable-wechat-notification-recovery.md).
...
**Adding a durable package-owned outbox** was initially deferred to keep a simple local signal over OpenClaw idempotency. The [outbox decision](../bug-fix/2026-09-11-durable-wechat-notification-recovery.md) supersedes this durability choice after channel failures demonstrated the recovery gap.
...
- At most the configured number of external subprocesses run concurrently, and pending plus queued retention cannot grow beyond its configured cap. Each delivered terminal still invokes one sender after coalescing, retention overflow favors recent completions, and sender failure does not alter the durable turn result.
- Delivery persistence, bounded retries and unknown outcomes follow the [outbox decision](../bug-fix/2026-09-11-durable-wechat-notification-recovery.md).
