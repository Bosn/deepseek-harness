...
> 范围：完成的轮次以其产出文件收尾的那一行、读得出是链接的文件路径链接，以及 Host 回退路径对浏览器可渲染文档优先选用默认浏览器。后来取代本笔记最初 HTTP 排除项的可选远端浏览器路径记录在[工作区文件源](2026-08-30-workspace-file-origin.zh.md)中。
...
**Host 打开器是默认回退路径，并且优先选用默认浏览器。** `session/openWorkspacePath` 把路径交给操作系统，得到的是真实浏览器里的一份 `file://` 文档：页面能力完整，且够不到 `/api`——因为 `file://` 文档与它并不同源。在所报告的那份产物上实测：`localStorage` 可用、主题切换生效、tabs 可切换，而对 API 的 `fetch` 失败。对浏览器能渲染的文档——`.html`、`.htm`、`.xhtml`、`.svg`——平台能够确定默认浏览器时，打开器解析的是默认**浏览器**而非该类型的默认应用，因为把 `.html` 绑给编辑器的开发者，否则点开一个产出的页面得到的会是源码。macOS 读取 LaunchServices 的 `https` 处理程序，桌面 Linux 读取 `$BROWSER`；无法确定浏览器时，两者都会回退到默认应用。Windows 使用其注册的文件关联，WSL 则先转换路径，再使用同一 Windows 交接。存在隐藏文件时，**在文件夹中显示**会把 `.` 经由同一 owner `openFile` 传递；它只在 loopback 页面的当前 `host.describe.canOpenPath` 允许原生打开时出现。其他部署会省略它；桌面探测误报时可配置 `nativeOpen: false`。

**最初的 HTTP 排除项已于 2026-08-30 被取代。** 远端 Web 部署现在可以显式选择一个已认证、独立发布的工作区文件源；未选择该能力的部署则逐字节保留 Host 路径。当前所有权与安全边界见[工作区文件源](2026-08-30-workspace-file-origin.zh.md)。

## 考虑过的替代方案

- **由独立源提供 `/f/<sessionId>/<segments…>`**——最初因为远端客户端当时不在产品范围而退役，在远端 Web 使用进入支持范围后以部署级选择恢复。恢复后的设计保留独立监听器、双 `realpath` 路径约束、流式读取和 URL 形状约定，并增加[工作区文件源](2026-08-30-workspace-file-origin.zh.md)所记录的当前浏览器会话认证边界。
