...
- Compaction treats HTTP 413 as request-size failure and treats `TIMEOUT` the same way only when the adapter-converted estimate, or the surfaced-request fallback, reaches `timeoutRecoveryBytes` (default 512 KiB). This prevents offloaded image bytes from turning an unrelated timeout into request-size recovery. Both bypass ordinary pressure and retained-tail policy, use the existing `maxOverflowRetries` budget, and retry only after durable surface replacement. Request-size recovery publishes a proactive byte budget at `learnedByteSafetyRatio` (default `0.75`) only after durable progress, scoped to that agent and exact provider/model route; semantic context overflow does not.
