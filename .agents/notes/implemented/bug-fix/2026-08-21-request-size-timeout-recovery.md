...
- Compaction treats HTTP 413 as request-size failure and treats `TIMEOUT` the same way only when the adapter-converted estimate, or the surfaced-request fallback, reaches `timeoutRecoveryBytes` (default 512 KiB). This prevents offloaded image bytes from turning an unrelated timeout into request-size recovery. The fallback byte estimator prices escaped JSON and framing for built-in content. Every positive confirmed-413 budget constrains its current summarizer; values below the 64 KiB proactive usefulness floor are not persisted. Both recovery paths bypass ordinary pressure and retained-tail policy, use the existing `maxOverflowRetries` budget, and retry only after durable surface replacement. Request-size recovery publishes a proactive byte budget at `learnedByteSafetyRatio` (default `0.75`) only after durable progress, scoped to that agent and exact provider/model route; semantic context overflow does not.
- `summarizationInputBytes` (default 512 KiB) caps the complete summarizer request from automatic pressure, explicit-region, and `compactNow()` entry points: routed header, replayed messages, and fixed instruction. Oversized ranges compact through earlier balanced transactions until every request fits. Each generated checkpoint enters the next bounded pass, so the complete replacement converges to one checkpoint instead of accumulating partition summaries. Partitions preserve tool-call/result pairs, every replaced message is represented in a bounded summarizer input, and an indivisible over-cap unit fails before provider dispatch.
