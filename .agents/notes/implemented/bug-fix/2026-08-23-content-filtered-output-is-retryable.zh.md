...
- `dsh-llm-pi-ai`（`classifyPiAiError`）：`content[_-\s]?filter(ed)?` 措辞与 `inappropriate content` 措辞映射为 `CONTENT_FILTERED`。该分支位于 HTTP 400 / `INVALID_REQUEST` 检查之前，因此即使网关把拒绝措辞为 400，也保留可重试类别，而不是落入永不重试的请求形状兜底。实际观察到的 dashscope-intl 原句和 pi-ai 合成的 `Provider finish_reason: content_filter` 都会命中该分类。
...
- 相关但未被取代：[空模型补全可重试](2026-07-24-empty-model-response-is-retryable.zh.md) 负责退化空 completion 类别；[pi-ai 传输截断分类](2026-07-22-pi-ai-transport-truncation-classification.zh.md) 负责同一函数中的传输措辞分类。