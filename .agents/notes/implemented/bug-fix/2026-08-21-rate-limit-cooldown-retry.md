...
- Both adapters reserve `QUOTA` for quota wording on non-429 statuses (a 402 insufficient balance) and classify every explicit HTTP 429 — quota-worded or not — as `RATE_LIMIT`, so `dsh-llm-retry` owns the wait. Pi-ai recovers the status from the SDK's flattened error message; its non-2xx response headers are not exposed to the adapter. Bounded by the schedule, a genuinely exhausted account still fails after about nine minutes.
