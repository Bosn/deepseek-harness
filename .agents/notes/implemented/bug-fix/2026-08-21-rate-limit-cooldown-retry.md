...
- `dsh-llm-retry` takes the schedule entry as the attempt's delay, floored at the provider `Retry-After` (valid, in timer range) and jittered by the shared ratio. In normal mode the schedule replaces `maxRetries` as the RATE_LIMIT budget (`min(maxRetries, length)`), with the schedule exhausted before the shared budget; always mode keeps retrying with exponential backoff past the schedule. The canonical policy key includes the schedule, so a changed schedule resets in-step retry history like any other policy change.
