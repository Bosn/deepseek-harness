...
- `dsh-llm-retry` takes the schedule entry as the attempt's delay, jittered by the shared ratio but never below the entry or a valid provider `Retry-After` (in timer range; an out-of-range value is ignored). In normal mode the schedule replaces `maxRetries` as the RATE_LIMIT budget (`min(maxRetries, length)`), with the schedule exhausted before the shared budget; always mode keeps retrying with exponential backoff past the schedule. The canonical policy key includes the schedule, so a changed schedule resets in-step retry history like any other policy change.
