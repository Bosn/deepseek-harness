...
- The resolved-policy fields and canonical `policyKey` include the schedule, so the unified `snapshots/session` retry scenarios and `apps/cli/tests/profiles/headless` provider-retry fixture pin short schedules; their recorded `policyKey` strings embed the pinned arrays.
